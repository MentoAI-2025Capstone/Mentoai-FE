...
// [수정됨]
// (GET /auth/me) -> (GET /users/{userId})
// API 명세서에 있는 '사용자 조회' 엔드포인트를 사용하도록 변경합니다.
export const checkCurrentUser = async () => {
  try {
    // 세션 스토리지에서 userId를 가져옵니다.
    const { userId } = getAuthData();
    if (!userId) {
      throw new Error("User ID not found in sessionStorage");
    }

    // /auth/me 대신 /users/{userId} 호출
    const response = await apiClient.get(`/users/${userId}`, getAuthHeaders());
    return { success: true, data: response.data }; // User 스키마 반환
  } catch (error) {
    console.warn("GET /users/{userId} 실패 (401 예상):", error.response);
