...
import axios from 'axios'; // Google API 요청용
import './Page.css';
...
const loginToBackend = async (googleUserData) => {
  try {
    ...
    // apiClient를 사용하여 MentoAI 백엔드에 요청
...
function AuthPage() {
  ...
  const handleGoogleLogin = useGoogleLogin({
    ...
    onSuccess: async (googleTokenResponse) => {
      ...
      try {
        // 2. Google userinfo API 호출 (순수 axios 사용)
        ...
        // 3. 백엔드 POST /users API 호출 (apiClient 사용)
        ...
        if (error.code === 'ERR_NETWORK' || alertMessage.includes('Network Error')) {
          ...
          alert('로그인에 실패했습니다. (서버 응답 시간 초과: 120초)');
        } else {
